...
{
   public static class SpellingUtil
    {
       ...
       public static string GetSpelling(string chineseChars)
       {
       ...
       public static string GetPrefixLetters(string chineseChars)
       { 
           byte[] byteAray = System.Text.Encoding.Default.GetBytes(chineseChars);
           int byteValue;
           StringBuilder sb = new StringBuilder(chineseChars.Length * 4);
           for (int i = 0; i < byteAray.Length; i++)
           {
               byteValue = (int)byteAray[i];
               if (byteValue >160)
               {
                   byteValue = byteValue * 256 + byteAray[++i] - 65536;
